...
|---|:--:|---|
| [existing-solution-assessment.md](existing-solution-assessment.md) | ✅ | Present-tense assessment of the repo (spec §7): architecture, projects, translation / database / API / Redis / auth architecture, pipelines, tests, dependencies, and the removed features (API keys, webhooks, RESX import, the language catalogue) with why. |
...
| [api.md](api.md) | ✅ | REST reference — the **Consumer API** (one ETag-aware route) and the **Management API** (projects, languages, keys, strings, review, review-bulk, publish + preview, export, import, grid, categories, dashboard, missing, history), status codes, the RFC 7807 mapping, health. |
...
| [import-export.md](import-export.md) | ✅ | The translator work-file round-trip: `GET .../export` (CSV / XLSX, query params, filenames) and `POST .../import` (wide vs narrow header shape, `contentBase64` for xlsx, blank-cell = skip, dry-run), worked examples, the send-to-a-translator workflow, the 5 MB cap, the Admin UI Export / Import screens. |
...
| [migration.md](migration.md) | ✅ | Moving an existing solution's old translation data in: identify → map to `project.key` + categories + languages → import via `POST /api/projects/{project}/import` (json/flat/csv/xlsx, incl. wide multi-language sheets) or a script → validate → retire. "Do not auto-delete production data." |
