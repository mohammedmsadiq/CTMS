...
/// <summary>
/// Persists the ASP.NET Core Data Protection key ring to Redis so every replica shares one set
/// of keys (antiforgery tokens, auth-cookie protection, anything else that calls
...
/// </summary>
/// <remarks>
/// Uses the same <c>ConnectionStrings:Redis</c> connection as the bundle cache. When Redis is
/// not configured it falls back to the framework default (keys in a local directory, ephemeral)
/// and logs an info line — the same degrade-quietly pattern as <c>CacheModeLogger</c>.
/// </remarks>
